...
StorageService.setCacheScope('instructor:account-one');
StorageService.saveAssessments([{ id: 'scope-one', periodId: 'period-one', studentId: 'student-one', finalScore: 0 }]);
StorageService.setCacheScope('instructor:account-two');
assert.deepEqual(plain(StorageService.getAssessments()), [], 'A different instructor must not receive the first account cache or seeded demo rows');
StorageService.saveAssessments([{ id: 'scope-two', periodId: 'period-two', studentId: 'student-two', finalScore: 88 }]);
StorageService.setCacheScope('instructor:account-one');
assert.equal(StorageService.getAssessments()[0].id, 'scope-one');
StorageService.setCacheScope('student:student-one:period-one');
StorageService.saveAttendance([{ id: 'period-cache', periodId: 'period-one', studentId: 'student-one', percentage: 0 }]);
StorageService.setCacheScope('student:student-one:period-two');
assert.deepEqual(plain(StorageService.getAttendance()), [], 'Student caches must also be isolated by practice period');
StorageService.setCacheScope('local');
