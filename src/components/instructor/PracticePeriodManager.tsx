...
export const PracticePeriodManager: React.FC = () => {
  ...
  const handleEditStartDateChange = (val: string) => {
    ...
    if (autoCalculateEndDate && val) {
      const end = computePeriodEndDate(val, 5);
      setEditEndDateInput(end);
      setEditStatusInput(computePeriodStatus(val, end, getWitaDateString()));
    } else if (val && editEndDateInput) {
      setEditStatusInput(computePeriodStatus(val, editEndDateInput, getWitaDateString()));
    }
  };

  const handleEditEndDateChange = (val: string) => {
    setEditEndDateInput(val);
    setAutoCalculateEndDate(false);
    if (editStartDateInput && val) {
      setEditStatusInput(computePeriodStatus(editStartDateInput, val, getWitaDateString()));
    }
  };
        ...
        <ModalPortal>
          ...
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleSaveNewPeriod} className="p-6 space-y-4">
              ...
              {/* 5-day Calculation Alert */}
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-xs text-blue-900 space-y-1">
                <p className="font-bold flex items-center gap-1">
                  <Sparkles className="w-3.5 h-3.5 text-blue-600" />
                  <span>Kalkulasi 5 Hari Praktik:</span>
                </p>
                <p className="text-[11px] text-blue-800">
                  Tanggal Selesai: <strong>{calculatedEndDate}</strong> ({formatPeriodRange(startDateInput, calculatedEndDate)})
                </p>
              </div>
            ...
            <form onSubmit={handleSaveEditPeriod} className="p-6 space-y-4">
              ...
              <div className="bg-blue-50/80 border border-blue-200/80 rounded-xl p-3 text-xs text-blue-900 space-y-2">
                    ...
                    <input
                      ...
                      onChange={e => {
                        ...
                        if (checked && editStartDateInput) {
                          const end = computePeriodEndDate(editStartDateInput, 5);
                          setEditEndDateInput(end);
                          setEditStatusInput(computePeriodStatus(editStartDateInput, end, getWitaDateString()));
                        }
                      }}
                      className="w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500"
                    />
                    <span>Kalkulasi 5 Hari Kerja (Senin–Jumat)</span>
                  </label>
                  <button
                    type="button"
                    onClick={() => {
                ...
                <p className="text-[11px] text-blue-800 flex items-center gap-1.5">
                  ...
                  <span>
                    Durasi: <strong>{formatPeriodRange(editStartDateInput, editEndDateInput)}</strong>
                  </span>
                </p>
              </div>
