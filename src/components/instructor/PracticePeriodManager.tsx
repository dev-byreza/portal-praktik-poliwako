...
import {
  ...
  getWitaDateString,
  computePeriodStatus,
  getPeriodStatusAutoInfo
} from '../../utils/dateUtils';
import { useNetworkTime } from '../../services/networkTimeService';
...
export const PracticePeriodManager: React.FC = () => {
  ...
  const networkTime = useNetworkTime();
  ...
  const [editAutoStatus, setEditAutoStatus] = useState<boolean>(true);
  ...
  // Live auto status evaluation for edit modal based on internet date
  const liveAutoStatusInfo = useMemo(() => {
    ...
    return getPeriodStatusAutoInfo(editStartDateInput, finalEndDate, networkTime.dateString);
  }, [editStartDateInput, editEndDateInput, autoCalculateEndDate, networkTime.dateString]);
  ...
  const handleOpenEditPeriod = (periodToEdit?: PracticePeriod) => {
    ...
    if (!target) return;
    const isAuto = target.autoStatus !== false;
    setEditNameInput(target.name);
    setEditStartDateInput(target.startDate);
    setEditEndDateInput(target.endDate);
    setEditAutoStatus(isAuto);
    if (isAuto) {
      setEditStatusInput(computePeriodStatus(target.startDate, target.endDate, networkTime.dateString));
    } else {
  ...
  const handleSaveEditPeriod = (e: React.FormEvent) => {
    ...
    const finalStatus = editAutoStatus
      ? computePeriodStatus(editStartDateInput, finalEndDate, networkTime.dateString)
      : editStatusInput;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          {/* Live Realtime Internet Clock Indicator */}
          <div className="px-3 py-2 bg-slate-900 text-white rounded-2xl border border-slate-800 flex items-center gap-2.5 text-xs shadow-xs">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-1.5 text-[9px] text-slate-400 font-bold uppercase tracking-wider">
                ...
                <span>Waktu Realtime WITA</span>
                <span className="text-[8px] px-1 py-0.2 bg-slate-800 text-slate-300 rounded border border-slate-700">
                  {networkTime.source === 'local_fallback' ? 'Lokal' : 'Internet'}
                </span>
              </div>
              <span className="text-xs font-mono font-bold text-cyan-300">
                {networkTime.formattedFull}
              </span>
            </div>
          </div>

          {/* Sync All Periods Button */}
          <button
            type="button"
            onClick={async () => {
              setIsSyncingAll(true);
              try {
                await syncAllPeriodsStatus();
              } finally {
                setIsSyncingAll(false);
              }
            }}
            disabled={isSyncingAll}
            className="px-3.5 py-2.5 bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold text-xs rounded-xl border border-blue-200 transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50 shadow-xs"
            title="Sinkronkan status semua gelombang dengan kalender dan waktu internet realtime"
          >
            <RefreshCw className={`w-3.5 h-3.5 text-blue-600 ${isSyncingAll ? 'animate-spin' : ''}`} />
            <span>Sinkron Status Otomatis</span>
          </button>

          <button
            onClick={handleOpenCreatePeriod}
            className="px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-blue-600/30 transition-all flex items-center gap-2 shrink-0 cursor-pointer"
          >
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {period.autoStatus !== false && (
                          <span className="inline-flex items-center gap-0.5 text-[8px] font-bold text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded border border-blue-200" title="Status tersinkron otomatis dengan kalender internet">
                      ...
                      <p className="text-[10px] text-slate-400 mt-0.5 font-medium">
                        • {getPeriodStatusAutoInfo(period.startDate, period.endDate, networkTime.dateString).relativeNotice}
                      </p>
                    </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleSaveNewPeriod} className="p-6 space-y-4">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-xs text-blue-900 space-y-1.5">
                ...
                <p className="text-[11px] text-blue-950 flex items-center gap-1 font-semibold pt-1 border-t border-blue-200/60">
                  <Zap className="w-3 h-3 text-amber-500 fill-amber-500" />
                  <span>
                    Status Otomatis Internet: <strong>{getPeriodStatusAutoInfo(startDateInput, calculatedEndDate, networkTime.dateString).badgeLabel}</strong> ({getPeriodStatusAutoInfo(startDateInput, calculatedEndDate, networkTime.dateString).relativeNotice})
                  </span>
                </p>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="inline-flex p-0.5 bg-slate-100 rounded-lg border border-slate-200">
                    <button
                      type="button"
                      onClick={() => setEditAutoStatus(true)}
                    ...
                    >
                      <Zap className="w-3 h-3 text-amber-500 fill-amber-500" />
                      <span>Otomatis (Internet)</span>
                    </button>
                    <button
                      type="button"
                      onClick={() => setEditAutoStatus(false)}
                      className={`px-2.5 py-1 rounded-md text-[10px] font-bold transition-all flex items-center gap-1 cursor-pointer ${
                        !editAutoStatus
                ...
                {editAutoStatus ? (
                  <div className="bg-gradient-to-br from-slate-900 via-slate-900 to-slate-950 rounded-2xl p-4 text-white border border-slate-800 shadow-sm space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">
                          Status Otomatis (Terkalkulasi Realtime)
                        </span>
                        <div className="flex items-center gap-2 mt-1">
                          {liveAutoStatusInfo.status === 'ACTIVE' && (
                            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                              🟢 Aktif (Sedang Berjalan)
                            </span>
                          )}
                          {liveAutoStatusInfo.status === 'UPCOMING' && (
                            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-amber-500/20 text-amber-300 border border-amber-500/30">
                              <span className="w-2 h-2 rounded-full bg-amber-400" />
                              🟡 Akan Datang (Terjadwal)
                            </span>
                          )}
                          {liveAutoStatusInfo.status === 'COMPLETED' && (
                            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-slate-500/20 text-slate-300 border border-slate-500/30">
                              <span className="w-2 h-2 rounded-full bg-slate-400" />
                              ⚪ Selesai (Arsip Periode)
                            </span>
                          )}
                        </div>
                      </div>
                      <span className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-blue-500/20 text-cyan-300 border border-cyan-500/30 shrink-0">
                        {liveAutoStatusInfo.relativeNotice}
                      </span>
                    </div>

                    <p className="text-[11px] text-slate-300 leading-relaxed">
                      {liveAutoStatusInfo.description} Status gelombang ini sinkron secara realtime dengan kalender internet dan akan aktif/selesai tepat waktu secara otomatis tanpa perlu diubah manual.
                    </p>
                    ...
                    <div className="pt-2 border-t border-slate-800/80 flex flex-wrap items-center justify-between gap-2 text-[10px] text-slate-400">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span>Internet Realtime (WITA): <strong className="text-slate-200">{networkTime.dateString} {networkTime.timeString}</strong></span>
                      </div>
                      <span className="text-slate-400 font-mono text-[9px] bg-slate-800 px-1.5 py-0.5 rounded">Sumber: {networkTime.source}</span>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-2">
                    ...
                    <p className="text-[10px] text-amber-700 bg-amber-50 p-2.5 rounded-xl border border-amber-200 leading-relaxed">
                      ⚠️ <strong>Mode Override Manual Aktif:</strong> Status gelombang ini dipaksa manual oleh instruktur dan tidak akan berganti otomatis mengikuti kalender internet sampai Anda mengaktifkan kembali <em>Otomatis (Internet)</em>.
                    </p>
                  </div>
                )}
              </div>
