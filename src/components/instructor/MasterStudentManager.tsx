...
export const MasterStudentManager: React.FC = () => {
  ...
  const [activationCodeInfo, setActivationCodeInfo] = useState<{ student: Student; code: string } | null>(null);
  ...
  const handleIssueActivationCode = async (student: Student) => {
    try {
      const code = await resetStudentPassword(student.id);
      if (code) setActivationCodeInfo({ student, code });
    } catch (error: any) {
      showToast('Kode Aktivasi Gagal Dibuat', error?.message || 'Server tidak dapat menerbitkan kode aktivasi.', 'error');
    }
  };
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          onClick={() => setConfirmState({
                            title: std.hasCreatedPassword ? 'Reset Password Mahasiswa?' : 'Terbitkan Kode Aktivasi?',
                            message: std.hasCreatedPassword
                              ? `Password ${std.name} (${std.nim}) akan dicabut dan semua sesi lama dibatalkan. Kode aktivasi baru berlaku 24 jam.`
                              : `Buat kode aktivasi sekali pakai untuk ${std.name} (${std.nim}). Kode berlaku 24 jam dan harus disampaikan melalui kanal institusi tepercaya.`,
                            confirmLabel: std.hasCreatedPassword ? 'Reset & Terbitkan Kode' : 'Terbitkan Kode',
                            variant: 'warning',
                            onConfirm: () => { void handleIssueActivationCode(std); },
                          })}
                          className="rounded-lg p-1.5 text-slate-400 transition-colors hover:bg-amber-50 hover:text-amber-600"
                          title={std.hasCreatedPassword ? 'Reset password dan cabut sesi lama' : 'Terbitkan kode aktivasi satu kali'}
                          aria-label={std.hasCreatedPassword ? `Reset password ${std.name}` : `Terbitkan kode aktivasi untuk ${std.name}`}
                        >
      ...
      {activationCodeInfo && (
        <ModalPortal>
          <div className="fixed inset-0 z-[75] flex items-center justify-center bg-slate-950/65 p-4 backdrop-blur-sm">
            <section role="dialog" aria-modal="true" aria-labelledby="activation-code-title" className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
              <div className="mb-4 flex items-start justify-between gap-4">
                <div>
                  <h2 id="activation-code-title" className="text-base font-bold text-slate-900">Kode Aktivasi Sekali Pakai</h2>
                  <p className="mt-1 text-xs text-slate-600">{activationCodeInfo.student.name} · NIM {activationCodeInfo.student.nim}</p>
                </div>
                <button type="button" onClick={() => setActivationCodeInfo(null)} className="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100" aria-label="Tutup kode">×</button>
              </div>
              <p className="text-xs leading-relaxed text-slate-600">Sampaikan kode ini langsung kepada mahasiswa melalui kanal institusi tepercaya. Kode hanya berlaku 24 jam dan hanya dapat digunakan satu kali.</p>
              <code className="mt-4 block break-all rounded-xl border border-amber-200 bg-amber-50 p-4 text-center font-mono text-lg font-bold tracking-[0.16em] text-amber-950">{activationCodeInfo.code}</code>
              <div className="mt-5 flex justify-end gap-2">
                <button type="button" onClick={() => setActivationCodeInfo(null)} className="rounded-xl bg-slate-100 px-4 py-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-200">Tutup</button>
                <button type="button" onClick={async () => {
                  try {
                    await navigator.clipboard.writeText(activationCodeInfo.code);
                    showToast('Kode Disalin', 'Simpan dan sampaikan kode melalui kanal tepercaya.', 'success');
                  } catch {
                    showToast('Gagal Menyalin', 'Salin kode secara manual.', 'error');
                  }
                }} className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-bold text-white hover:bg-blue-500"><Copy className="h-3.5 w-3.5" />Salin Kode</button>
              </div>
            </section>
          </div>
        </ModalPortal>
